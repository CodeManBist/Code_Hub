...
async function commitRepo(message, argv = {}) {
  ...
  try {
    ...
    if (options.stateBackend === "s3") {
      ...
      const stagedObjects = (listResult.Contents || []).filter(
        (entry) => entry.Key && entry.Key !== stagingPrefix
      );
      ...
      for (const entry of stagedObjects) {
        ...
        const relativePath = key.replace(stagingPrefix, "");

        await s3.send(
          new CopyObjectCommand({
            Bucket: S3_BUCKET,
            CopySource: `${S3_BUCKET}/${key}`,
            Key: `${commitPrefix}${relativePath}`,
          })
        );
      }
    ...
    const files = await walkDirectory(stagedPath);
