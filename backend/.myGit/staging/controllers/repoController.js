...
async function createRepository(req, res) {
    ...
    try {
        ...
        const initialFiles = (content || []).map((item) => {
            const [filePath, ...body] = String(item).split(":");
            return { path: filePath.trim(), content: body.length ? body.join(":").trim() : `// ${filePath.trim()}\n` };
        }).filter((file) => file.path);
        if (!initialFiles.some((file) => /^readme\.md$/i.test(file.path))) {
            initialFiles.unshift({ path: "README.md", content: `# ${name}\n\n${description || "Welcome to this CodeHub repository."}\n` });
        }
        const newRepository = new Repository ({
            ...
            content,
            issues,
            commits: [{ id: randomUUID(), message: "Initial commit", author: "CodeHub", files: initialFiles }]
        });
...
async function updateRepositoryById(req, res) {
    ...
    try {
        const updateRepository = await Repository.findByIdAndUpdate(
            id,
            { ...(content !== undefined ? { content } : {}), ...(description !== undefined ? { description } : {}), ...(name !== undefined ? { name } : {}) },
            { new: true }
        );
        ...
        res.status(200).json({ message: "Repository updated successfully", repository: normalizeRepository(updateRepository) }); 
    } catch (error) {
